...
The supplied persistent token is treated as a credential and is never written to lifecycle logs. This differs from an ephemeral token generated by the gateway, which must be printed so an operator can reach the new share.

### How It Works

When sharing is enabled, the gateway adds a zrok listener. It can serve the same handler over local TCP, zrok, and Agora at once; omit `listen` when an overlay is enabled to avoid opening the optional local listener. From the handler's perspective, nothing changes -- it still receives `http.Request` objects and writes `http.Response` objects.
...
Operator-supplied access tokens are never logged. Lifecycle messages identify the provider, endpoint, or key source that owns the access instead.
