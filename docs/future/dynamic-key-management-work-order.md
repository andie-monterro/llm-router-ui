...
| `max_staleness > 0` and `≤ poll_interval + timeout` for any source | error — stage 3 implements this as `≤ poll_interval`, which is the same rule while the file source is the only one and has no `timeout`; **stage 4 must add the `+ timeout` term** when the HTTP source introduces one |
...
It also inherits one obligation from stage 3, in the same way stage 3 inherited the `max_staleness` guard from stage 2. The accommodation check is currently `max_staleness > poll_interval`, correct while the file source is the only source and has no timeout. The HTTP source introduces `timeout`, and the check must become `max_staleness > poll_interval + timeout` at that moment — otherwise `poll_interval: 30s, timeout: 5s, max_staleness: 31s` passes validation and then flaps, since a refresh that starts at the poll boundary can still be in flight when the deadline fires. That flapping configuration being a boot error rather than a production discovery is the whole point of the rule.
